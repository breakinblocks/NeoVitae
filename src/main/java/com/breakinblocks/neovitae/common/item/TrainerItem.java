...
public class TrainerItem extends Item {
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
        ...
        if (limits == null) {
            tooltip.add(Component.translatable("tooltip.neovitae.trainer.unconfigured").withStyle(ChatFormatting.GRAY));
            return;
        }

        tooltip.add(Component.translatable("tooltip.neovitae.trainer.plan").withStyle(ChatFormatting.GOLD));
        limits.limits().object2FloatEntrySet().stream()
                ...
                .forEach(entry -> {
                    ...
                    if (level <= 0) {
                        tooltip.add(Component.translatable("tooltip.neovitae.trainer.blocked", name)
                                .withStyle(ChatFormatting.RED));
                    } else {
                        tooltip.add(name.append(CommonComponents.SPACE)
                                .append(Component.literal(ChatUtil.toRoman(level)))
                                .withStyle(ChatFormatting.GRAY));
                    }
                });
        tooltip.add(Component.translatable(limits.allowOthers()
