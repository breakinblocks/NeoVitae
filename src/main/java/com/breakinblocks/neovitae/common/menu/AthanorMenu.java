...
public class AthanorMenu extends AbstractBlockEntityMenu<AthanorBlockEntity> {
    ...
    public AthanorMenu(int containerId, Inventory playerInventory, AthanorBlockEntity tile) {
        ...
        this.addSlot(new ResourceHandlerSlot(tile.athanorInv, tile.athanorInv::set, AthanorBlockEntity.TOOL_SLOT, 35, 55));

        for (int row = 0; row < 2; row++) {
            for (int col = 0; col < 3; col++) {
                int slotIdx = AthanorBlockEntity.INPUT_START + row * 3 + col;
                this.addSlot(new ResourceHandlerSlot(tile.athanorInv, tile.athanorInv::set, slotIdx, 33 + col * 18, 14 + row * 18));
            }
        }

        this.addSlot(new ResourceHandlerSlot(tile.athanorInv, tile.athanorInv::set, AthanorBlockEntity.INPUT_BUCKET_SLOT, 8, 18));
        this.addSlot(new ResourceHandlerSlot(tile.athanorInv, tile.athanorInv::set, AthanorBlockEntity.OUTPUT_BUCKET_SLOT, 152, 90));

        for (int i = 0; i < AthanorBlockEntity.NUM_OUTPUTS; i++) {
            this.addSlot(new ResourceHandlerSlot(tile.athanorInv, tile.athanorInv::set, AthanorBlockEntity.OUTPUT_SLOT + i, 116, 18 + i * 18) {
                @Override
