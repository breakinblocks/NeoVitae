...
public record CooldownEffect(Identifier id) implements LivingEntityEffect {
    public static final MapCodec<CooldownEffect> CODEC = RecordCodecBuilder.mapCodec(builder -> builder.group(
            Identifier.CODEC.fieldOf("id").forGetter(CooldownEffect::id)
    ).apply(builder, CooldownEffect::new));

    @Override
    public void apply(int upgradeLevel, Entity entity) {
        Map<Identifier, Double> data = entity.getData(NVDataAttachments.LIVING_ADDITIONAL.get());
