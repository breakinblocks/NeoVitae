...
public record ResetCooldownEffect(Identifier id, LevelBasedValue amounts, Optional<SentientEntityEffect> effect) implements SentientEntityEffect {
    public static final MapCodec<ResetCooldownEffect> CODEC = RecordCodecBuilder.mapCodec(builder -> builder.group(
            Identifier.CODEC.fieldOf("id").forGetter(ResetCooldownEffect::id),
    ...
    public void apply(int upgradeLevel, Entity entity) {
        Map<Identifier, Double> data = entity.getData(NVDataAttachments.SENTIENT_ADDITIONAL);
