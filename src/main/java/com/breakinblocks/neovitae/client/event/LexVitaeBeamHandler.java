...
public final class LexVitaeBeamHandler {
    ...
    public static void onMouseScroll(InputEvent.MouseScrollingEvent event) {
        ...
        if (delta == 0) return;
        ClientPacketDistributor.sendToServer(new LexCycleRadiusPayload(delta < 0 ? 1 : -1));
        event.setCanceled(true);
    }

    // 26.1 removed direct forwardImpulse/leftImpulse on ClientInput; vanilla itemUseSpeedMultiplier
    // path can't be cancelled cleanly without a Mixin, so the holding-beam slowdown remains.
    // Tracked in migration_alterations.md.
    ...
    public static void onClientTick(ClientTickEvent.Post event) {
        ...
        float t = player.tickCount + mc.getDeltaTracker().getGameTimeDeltaPartialTick(false);
