...
public class SanguineWardTests {
    ...
    private static Zombie attackerAt(GameTestHelper helper, Player player, double offsetZ) {
        ...
        zombie.moveTo(player.getX(), player.getY(), player.getZ() + offsetZ, 0.0f, 0.0f);
    ...
    public void wardBlocksProjectilesFromTheFront(GameTestHelper helper) {
    ...
    @GameTest(template = "empty_5x5x7", timeoutTicks = 80)
    public void wardDropsTridentsInsteadOfDestroyingThem(GameTestHelper helper) {
        helper.runAfterDelay(1, () -> {
            Player player = wardedPlayer(helper);
            BloodOrbItem.setShieldActive(player, true);
    ...
    @GameTest(template = "empty_5x5x7", timeoutTicks = 80)
    public void wardDropsThrowingDaggersInsteadOfDestroyingThem(GameTestHelper helper) {
        helper.runAfterDelay(1, () -> {
            Player player = wardedPlayer(helper);
            BloodOrbItem.setShieldActive(player, true);
    ...
    @GameTest(template = "empty_5x5x7", timeoutTicks = 80)
    public void wardIgnoresProjectilesFromBehind(GameTestHelper helper) {
        helper.runAfterDelay(1, () -> {
            Player player = wardedPlayer(helper);
            BloodOrbItem.setShieldActive(player, true);
            Arrow arrow = arrowAt(helper, player, -2.0);
    ...
    @GameTest(template = "empty_5x5x7", timeoutTicks = 80)
    public void wardDoesNotBlockMelee(GameTestHelper helper) {
        helper.runAfterDelay(1, () -> {
            Player player = wardedPlayer(helper);
            BloodOrbItem.setShieldActive(player, true);
    ...
    @GameTest(template = "empty_5x5x7", timeoutTicks = 80)
    public void inactiveWardDoesNotBlock(GameTestHelper helper) {
        helper.runAfterDelay(1, () -> {
            Player player = wardedPlayer(helper);
            BloodOrbItem.setShieldActive(player, false);
            Arrow arrow = arrowAt(helper, player, 2.0);
