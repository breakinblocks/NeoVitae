...
public final class AlternatorLinkOverlayHandler {
    ...
    public static void onRenderLevel(RenderLevelStageEvent event) {
        ...
        for (DungeonAlternatorBlockEntity alternator : targets) {
            ...
            LevelRenderer.renderLineBox(poseStack, lines, new AABB(sourcePos).inflate(0.004), RED, GREEN, BLUE, ALPHA);
            Vec3 sourceCenter = Vec3.atCenterOf(sourcePos);
            for (BlockPos receiver : alternator.getReceivers()) {
                LevelRenderer.renderLineBox(poseStack, lines, new AABB(receiver).inflate(0.004), RED, GREEN, BLUE, ALPHA);
                drawLine(poseStack, lines, sourceCenter, Vec3.atCenterOf(receiver));
            }
        }
