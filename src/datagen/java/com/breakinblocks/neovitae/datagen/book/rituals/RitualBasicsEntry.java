...
public class RitualBasicsEntry extends EntryProvider {
    ...
    protected void generatePages() {
        ...
        this.pageText("- Enough Ritual Stones to build the Ritual."
                + "\n\n- (Recommended) A [#](8B0000)Ritual Diviner[#](). Although not required, it will make ritual construction significantly easier."
        ...
        this.page("troubleshooting", () -> BookTextPageModel.create()
                .withText(this.context().pageText()));
        this.pageText("If this does not occur, a few things may have gone wrong. If you instead get text saying 'You feel a push, but are too weak to complete this ritual', then you do not have enough [#](8B0000)LP[#]() in your [#](8B0000)Soul Network[#]() to activate the ritual."
        ...
        this.pageText("Remember that some rituals extend several blocks above and below the Master Ritual Stone. If you get no error, ensure that the activation crystal is bound to your soul network - this can be accomplished by pressing Use while holding it."
                + "\\\n\\\nIt is important to note that the crystal does not have to be bound to YOUR network - if you can get a hold of another player's crystal, you can activate rituals using their [#](8B0000)LP[#](). Guard yours well!");

        this.page("redstone", () -> BookTextPageModel.create()
                .withText(this.context().pageText()));
        this.pageText("One last note; all rituals respond to a [#](8B0000)redstone[#]() signal, so sticking a lever on the side of the Master Ritual Stone is a good way to deactivate it. You can combine this knowledge with some redstone automation to ensure your rituals shut down automatically long before your [#](8B0000)Soul Network[#]() runs dry.");
    }
