...
public class VitaeLinkBlockEntity extends BaseBlockEntity {
    ...
    public class Inv extends ItemStacksResourceHandler {
        ...
        public boolean isValid(int index, ItemResource resource) {
            return index == INPUT_SLOT && getStackInSlot(OUTPUT_SLOT).isEmpty();
        }

        @Override
        protected int getCapacity(int index, ItemResource resource) {
            return index == INPUT_SLOT ? 1 : super.getCapacity(index, resource);
        }

        @Override
        protected void onContentsChanged(int index, ItemStack previousContents) {
            if (index == INPUT_SLOT && !ignoreInputChange) {
                recipeDirty = true;
    ...
    private AraVitaeRecipe getRecipe() {
        ...
        int eff = effTier();
        if (in.isEmpty() || eff < 0 || !(level instanceof ServerLevel serverLevel)) {
            cachedRecipe = null;
            return null;
        }
        cachedRecipe = serverLevel.recipeAccess()
                .getRecipeFor(NVRecipes.ARA_VITAE_TYPE.get(), new AraVitaeInput(in, eff), serverLevel)
    ...
    private void completeCraft(AraVitaeTile altar, ItemStack input, AraVitaeRecipe recipe) {
        ...
        ItemStack result = recipe.assemble(ri);
    ...
    public static ResourceHandler<ItemResource> getItemHandler(VitaeLinkBlockEntity tile, @Nullable Direction side) {
        if (side == null) return tile.inv;
        return switch (side) {
            case DOWN -> RangedResourceHandler.of(tile.inv, OUTPUT_SLOT, OUTPUT_SLOT + 1);
            default -> RangedResourceHandler.of(tile.inv, INPUT_SLOT, INPUT_SLOT + 1);
        };
    }
    ...
    protected void loadAdditional(ValueInput tag) {
        ...
        craftTier = tag.getIntOr("craftTier", 0);
        progress = tag.getIntOr("progress", 0);
        craftedThisOccupancy = tag.getBooleanOr("crafted", false);
