...
/**
 * Patchouli processor for Potion Flask recipes.
 *
 * Example Page:
 * {
 *   "type": "neovitae:crafting_flask",
 *   "heading": "Title",
 *   "recipe": "recipe_id",
 *   "text": "Extra text."
 * }
...
public class PotionFlaskProcessor implements IComponentProcessor {
    ...
    public void setup(Level level, IVariableProvider variables) {
        ...
        Optional<RecipeHolder<FlaskRecipe>> recipeHolder = Minecraft.getInstance().level.getRecipeManager()
    ...
    public IVariable process(Level level, String key) {
        ...
        if (key.startsWith("input")) {
            ...
            if (recipe.getInput().size() > index) {
                ...
                // The flask itself as the final input
                return IVariable.from(recipe.getExampleFlask(), registries);
            } else {
                return null;
            }
        }
        return switch (key) {
            case "output" -> {
                ItemStack flaskStack = recipe.getExampleFlask();
                ItemStack outputStack = recipe.getOutput(flaskStack, recipe.getExampleEffects());
                yield IVariable.from(outputStack, registries);
            }
