...
public class RitualPlacer extends Ritual {
    ...
    public void performRitual(IMasterRitualStone masterRitualStone) {
        ...
        IItemHandler inventory = findAdjacentInventory(level, masterPos);
        if (inventory == null) return;

        int slotIndex = findBlockItemSlot(inventory);
        ...
        while (placed < budget && lookups < lookupBudget && steps < stepBudget) {
            ...
            ItemStack toPlace = inventory.getStackInSlot(slotIndex);
            if (toPlace.isEmpty() || !(toPlace.getItem() instanceof BlockItem blockItem)) {
                slotIndex = findBlockItemSlot(inventory);
    ...
    private int findBlockItemSlot(IItemHandler inventory) {
    ...
    public void readFromNBT(CompoundTag tag) {
        ...
        cursorIndex = tag.contains("fillCursor") ? tag.getLong("fillCursor") : 0L;
        cursorVolume = tag.contains("fillCursorVolume") ? tag.getLong("fillCursorVolume") : 0L;
    }
