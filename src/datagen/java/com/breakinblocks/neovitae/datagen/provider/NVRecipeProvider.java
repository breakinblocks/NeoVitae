...
public class NVRecipeProvider extends RecipeProvider {
    ...
    private void addVanillaCraftingRecipes(RecipeOutput output) {
        ShapedRecipeBuilder.shaped(RecipeCategory.MISC, NVItems.GUIDE_BOOK.get())
                .pattern(" r ")
                .pattern("rbr")
                .pattern(" r ")
        ...
        SimpleCookingRecipeBuilder.smelting(Ingredient.of(NVItems.DEMONITE_RAW.get()), RecipeCategory.MISC, NVItems.HELLFORGED_INGOT.get(), 0.7f, 200)
                .unlockedBy("has_raw_demonite", has(NVItems.DEMONITE_RAW.get()))
                .save(output, NeoVitae.rl("smelting/hellforged_ingot_from_raw"));

        // Raw Demonite blasting -> Hellforged Ingot
        SimpleCookingRecipeBuilder.blasting(Ingredient.of(NVItems.DEMONITE_RAW.get()), RecipeCategory.MISC, NVItems.HELLFORGED_INGOT.get(), 0.7f, 100)
                .unlockedBy("has_raw_demonite", has(NVItems.DEMONITE_RAW.get()))
        ...
        ShapedRecipeBuilder.shaped(RecipeCategory.TOOLS, NVItems.RITUAL_DIVINER_TENEBRAE.get())
                .pattern(" S ")
                .pattern("tdt")
                .pattern(" S ")
                .define('S', NVItems.TABULA_SPIRITUS.get())
    ...
    private void addAraVitaeRecipes(RecipeOutput output) {
        ...
        AltarRecipeBuilder.build(NVItems.INSCRIPTION_TOOL_TENEBRAE.get())
                ...
                .drain(10)
                .unlockedBy("has_demonic_slate", has(NVItems.TABULA_SPIRITUS.get()))
                .save(output, NeoVitae.rl("tenebrae_tool"));
    ...
    private void addHellfireForgeRecipes(RecipeOutput output) {
        ...
        ShapelessRecipeBuilder.shapeless(RecipeCategory.MISC, NVBlocks.OUTPUT_ROUTING_NODE.block().get())
                .requires(NVBlocks.INPUT_ROUTING_NODE.block().get())
                .unlockedBy("has_routing_node", has(NVBlocks.INPUT_ROUTING_NODE.block().get()))
                .save(output, NeoVitae.rl("output_routing_node_from_input"));

        ShapelessRecipeBuilder.shapeless(RecipeCategory.MISC, NVBlocks.INPUT_ROUTING_NODE.block().get())
                .requires(NVBlocks.OUTPUT_ROUTING_NODE.block().get())
                .unlockedBy("has_routing_node", has(NVBlocks.OUTPUT_ROUTING_NODE.block().get()))
                .save(output, NeoVitae.rl("input_routing_node_from_output"));
    ...
    private void addTabulaVitaeRecipes(RecipeOutput output) {
        ...
        Holder<SentientUpgrade> curiosSocket = registriesFuture.join()
                .lookupOrThrow(NVRegistries.Keys.SENTIENT_UPGRADES)
                .getOrThrow(SentientUpgrades.CURIOS_SOCKET);
        ItemStack curiosSocketTome = new ItemStack(NVItems.UPGRADE_TOME.get());
        curiosSocketTome.set(NVDataComponents.UPGRADE_TOME_DATA.get(), new UpgradeTome(curiosSocket, 1.0f));
        TabulaVitaeRecipeBuilder.build(curiosSocketTome)
                .input(Items.WRITTEN_BOOK)
                .input(Items.SHULKER_SHELL)
        ...
        ShapelessRecipeBuilder.shapeless(RecipeCategory.MISC, NVItems.COAL_DUST.get(), 2)
                .requires(Items.COAL)
                .requires(NVItems.PLANT_OIL.get())
                .unlockedBy("has_plant_oil", has(NVItems.PLANT_OIL.get()))
                .save(output, NeoVitae.rl("coal_dust_from_coal"));
    ...
    private void addAthanorRecipes(RecipeOutput output) {
        ...
        SimpleCookingRecipeBuilder.smelting(Ingredient.of(NVItems.HELLFORGED_DUST.get()), RecipeCategory.MISC, NVItems.HELLFORGED_INGOT.get(), 0.7f, 200)
        ...
        SimpleCookingRecipeBuilder.blasting(Ingredient.of(NVItems.HELLFORGED_DUST.get()), RecipeCategory.MISC, NVItems.HELLFORGED_INGOT.get(), 0.7f, 100)
                .unlockedBy("has_hellforged_dust", has(NVItems.HELLFORGED_DUST.get()))
