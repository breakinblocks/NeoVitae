...
public class BookAltarTierInfoPageRenderer extends BookPageRenderer<BookAltarTierInfoPage> {
    ...
    public void render(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        guiGraphics.text(this.font,
                Component.translatable("book.neovitae.altar_tier_info.header.tier"), TIER_X, y, 0xFF4A0080, false);
        guiGraphics.text(this.font,
                Component.translatable("book.neovitae.altar_tier_info.header.runes"), RUNE_X, y, 0xFF4A0080, false);
        y += ROW_HEIGHT + 2;

        for (TierRow row : collectTiers()) {
            guiGraphics.text(this.font, String.valueOf(row.tier + 1), TIER_X, y, 0xFF555555, false);
            guiGraphics.text(this.font, String.valueOf(row.runeBlocks), RUNE_X, y, 0xFF555555, false);
            y += ROW_HEIGHT;
        }
    }
