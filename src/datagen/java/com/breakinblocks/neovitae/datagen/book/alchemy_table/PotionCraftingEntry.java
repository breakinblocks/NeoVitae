...
public class PotionCraftingEntry extends EntryProvider {
    ...
    protected void generatePages() {
        ...
        this.pageText("The basics of **Potion Crafting** are known to all and sundry throughout the land. "
        ...
        this.pageText("Craft the **Alchemy Flask** in the Blood Altar (recipe: neovitae:blood_altar/alchemy_flask). "
        ...
        this.page("catalysts_intro", () -> BookTextPageModel.create()
                .withText(this.context().pageText()));
        this.pageText("Next up, we have **Catalysts**. There are a few kinds of catalyst, with a few different "
                + "effects, so let's start off with the simplest three - **Simple Catalyst**, **Small Power Catalyst**, "
                + "and **Small Lengthening Catalyst**. Of these, the first is the most straightforward - it provides "
                + "a base to almost every Alchemical Potion, much as **Nether Wart** does for standard Potions.");

        this.page("simple_catalyst", () -> BookTextPageModel.create()
                .withTitle(this.context().pageTitle())
                .withText(this.context().pageText()));
        this.pageTitle("Simple Catalyst");
        this.pageText("Craft the **Simple Catalyst** in the Alchemy Table (recipe: neovitae:alchemytable/simple_catalyst). "
        ...
        this.pageText("Craft the **Combinational Catalyst** in the Alchemy Table (recipe: neovitae:alchemytable/combinational). "
                + "**Combinational Catalysts** are a little more niche - they take two defined effects and "
                + "synthesise a third, related effect from them. For example, a Flask with **Suspended** and "
                + "**Levitation** would, when brewed with this catalyst, produce a Flask of **Flight**.");

        this.page("filling_agent", () -> BookTextPageModel.create()
                .withTitle(this.context().pageTitle())
                .withText(this.context().pageText()));
        this.pageTitle("Weak Filling Agent");
        this.pageText("Craft the **Weak Filling Agent** in the Alchemy Table (recipe: neovitae:alchemytable/weak_filling). "
                ...
                + "If you wish to preserve a different effect, consider using the **Simple Cycling Catalyst**.");

        this.page("cycling_catalyst", () -> BookTextPageModel.create()
                .withTitle(this.context().pageTitle())
                .withText(this.context().pageText()));
        this.pageTitle("Simple Cycling Catalyst");
        this.pageText("Craft the **Simple Cycling Catalyst** in the Alchemy Table (recipe: neovitae:alchemytable/cycling_catalyst). "
        ...
        this.pageText("If the potions you've crafted so far just aren't enough for you, then perhaps you need "
                + "to do a little exploring? With some **Glow berries**, some **Cobbled Deepslate** and a sprinkling "
                + "of **Hellforged Sand**, you can boost the duration and power of your flasks significantly.");

        this.page("strengthened_catalyst", () -> BookTextPageModel.create()
                .withTitle(this.context().pageTitle())
                .withText(this.context().pageText()));
        this.pageTitle("Strengthened Catalyst");
        this.pageText("Craft the **Strengthened Catalyst** in the Alchemy Table (recipe: neovitae:alchemytable/strengthened_catalyst). "
        ...
        this.pageText("Craft the **Standard Lengthening Catalyst** in the Alchemy Table (recipe: neovitae:alchemytable/average_lengthening). "
        ...
        this.pageText("Craft the **Standard Power Catalyst** in the Alchemy Table (recipe: neovitae:alchemytable/average_power). "
