...
public class DisenchantCategory implements IRecipeCategory<DisenchantJEIRecipe> {
    public static final RecipeType<DisenchantJEIRecipe> RECIPE_TYPE =
            RecipeType.create(NeoVitae.MODID, "disenchant", DisenchantJEIRecipe.class);
    ...
    @Override public RecipeType<DisenchantJEIRecipe> getRecipeType() { return RECIPE_TYPE; }
    ...
    public void setRecipe(@Nonnull IRecipeLayoutBuilder builder, @Nonnull DisenchantJEIRecipe recipe, @Nonnull IFocusGroup focuses) {
        ...
        builder.addSlot(RecipeIngredientRole.INPUT, 20, ROW + 1).addItemStack(new ItemStack(Items.BOOK));

        IRecipeSlotBuilder fluid = builder.addSlot(RecipeIngredientRole.INPUT, 38, ROW + 1);
        fluid.addFluidStack(NVFluids.ESSENTIA_VITAE_SOURCE.get(), 100);
        fluid.addRichTooltipCallback((view, tooltip) -> tooltip.add(Component.literal("100 mB")));

        IRecipeSlotBuilder catalyst = builder.addSlot(RecipeIngredientRole.CATALYST, 59, ROW + 1);
        catalyst.addItemStack(new ItemStack(NVItems.SANGUINE_REVERTER.get()));

        builder.addSlot(RecipeIngredientRole.OUTPUT, 100, ROW + 1).addItemStacks(recipe.enchantedBooks());
    }

    @Override
    public void draw(DisenchantJEIRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
        Minecraft mc = Minecraft.getInstance();
        guiGraphics.drawString(mc.font, Component.translatable("jei.neovitae.disenchant.any_item"), 2, 2, GREEN, false);
        ...
        int cy = ROW + 22;
        guiGraphics.drawString(mc.font, Component.translatable("jei.neovitae.disenchant.per_enchant"), 2, cy, 0xAAAAAA, true);
        guiGraphics.fill(2, cy + 11, 6, cy + 15, RAW_COLOR);
        guiGraphics.drawString(mc.font, Component.translatable("jei.neovitae.disenchant.spiritus"), 8, cy + 10, 0xFFFFFF, true);
        guiGraphics.drawString(mc.font, Component.translatable("jei.neovitae.disenchant.ev"), 2, cy + 20, 0xFFFFFF, true);
    }

    @Override
    public ResourceLocation getRegistryName(DisenchantJEIRecipe recipe) {
