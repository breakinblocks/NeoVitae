...
public class SigilItem extends Item implements IBindable, IActivatable, ISigil {
        ...
        if (level != null && isToggleable(stack, level)) {
            ...
            if (getActivated(stack)) {
                ...
                if (upkeep > 0) {
                    ...
                    tooltip.accept(Component.translatable("tooltip.neovitae.sigil.upkeep", upkeep, secondsLabel)
                            .withStyle(ChatFormatting.GRAY));
                }
            }
        }
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (!ISigil.canPayFor(binding, player,
                getReducedCost(getLpCost(stack, level, SigilType.UseContext.BLOCK), player))) {
            return InteractionResult.CONSUME;
        }
